...
function AddBookModal() {
	...
	const addBookNameSchema = Yup.object().shape({
		book_name: Yup.string('Book name should be in string').required('Book name requied'),
	});
		...
		<div>
			<CModal id={'add-book-modal'} title={'Add Book'}>
				<div className="flex flex-col relative mb-9">
					...
					<input type="text" name="book_name" className="input-el" onChange={handleInputChange} ref={bookNameRef} />

					{error.book_name && <span className="error">{error.book_name}</span>}
				</div>

				<CButton className="w-[30%] flex justify-center mx-auto" onClick={handleBookAdd} isLoading={useSelector((state) => state.loader.isLoading)}>
					Submit
				</CButton>
			</CModal>
		</div>
