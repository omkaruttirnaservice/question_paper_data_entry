...
function AddTopicFormModal() {
	...
	const { data: _formData, subjectsList } = useSelector((state) => state.questionForm);

	const addTopicSchema = Yup.object().shape({
		topic_name: Yup.string('Topic name should be in string').required('Topic name requied'),
	});
			...
			<CModal id={'add-topic-modal'} title={'Add Topic'}>
				<div className="flex flex-col gap-3">
					...
					<div className="flex flex-col relative mb-6">
						...
						<input type="text" className="input-el mt-2" name="topic_name" ref={topicNameRef} onChange={handleInputChange} />

						{error.topic_name && <span className="error">{error.topic_name}</span>}
					</div>
