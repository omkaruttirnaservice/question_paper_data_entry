...
import { QuestionFormActions, getSubjectsListThunk } from '../../Store/question-form-slice.jsx';
									...
									<tr key={subject.id}>
										...
										<td>
											...
											{isSubjectNameEdit && subject.id === editId && (
												<>
													<input ref={editedSubjectName} className="form-control w-50 d-inline-block p-0 ms-2" />

													<CButton
														varient="btn--success"
														icon={<FaRegFloppyDisk />}
														onClick={handleSaveEditedSubjectName.bind(null, subject.id)}></CButton>

													<CButton varient="btn--danger" icon={<IoCloseSharp />} onClick={() => setIsSubjectNameEdit(false)}></CButton>
												</>
											)}
										</td>
										<td className="text-center">{subject.que_count}</td>
										<td className="text-center">
											<CButton varient="btn--success" icon={<FaRegTrashAlt />} onClick={() => handleDeleteSubject(subject.id)}></CButton>

											<CButton varient="btn--danger" icon={<GoPencil />} onClick={handleSubjectEdit.bind(null, subject.id)}></CButton>
										</td>
									</tr>
								);
							})}
